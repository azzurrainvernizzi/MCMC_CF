function [data_bayes_CF] = MCMC_CF_cluster(data_sub)
...
for q =1:size(tSeries_target,2) % nr of voxels across target region
    ...
    err = varY;% set error matrix to variance of Y
    ...
    for k=1:n_samples % number of iterations for MCMC
        ...
        [xi_current,VE_current,post_current,prior_current,loglike_current] = compute_mcmc_predictions(Y,tSeries_source,d_current,l_sigma,r_min,radius,how_beta,l_beta);
        [xi_proposal,VE_proposal,post_proposal,prior_proposal,loglike_proposal] = compute_mcmc_predictions(Y,tSeries_source,d_proposal,l_sigma_proposal,r_min,radius,how_beta,l_beta);
        ...
        % compute a posterior for it.
         elseif how_beta==1 
             ...
             [xi_current,VE_current,post_current,prior_current,loglike_current] = compute_mcmc_predictions(Y,tSeries_source,d_current,l_sigma,r_min,radius,how_beta,l_beta);
             [xi_proposal,VE_proposal,post_proposal,prior_proposal,loglike_proposal] = compute_mcmc_predictions(Y,tSeries_source,d_proposal,l_sigma_proposal,r_min,radius,how_beta,l_beta_proposal);
...
[best_fit{q},post_dist_b{q},loglikelihood_b{q},prior_dist_b{q},posterior_latent_b{q},posterior_b{q},VE_b{q}] = compute_burn_in_cf (post_dist,loglikelihood,prior_dist,posterior_latent,posterior,VE,burn_in,perc_burn_in);
...
data_bayes_CF = struct('best_fit',best_fit,'post_dist',post_dist_b,'loglikelihood', loglikelihood_b, 'prior_dist', prior_dist_b,'posterior_b',posterior_b, 'posterior_latent_b', posterior_latent_b);
