function [best_fit,post_dist_b,loglikelihood_b,prior_dist_b,posterior_latent_b,posterior_b,VE_b] = compute_burn_in_cf_Model (post_dist,loglikelihood,prior_dist,posterior_latent,posterior,VE,burn_in,perc_burn_in,AICBIC)
%---------------------------------------------------
...
if burn_in==1 % applying burn-in -- remove the first 1000 of elements on 10000 runs
    if size(posterior_latent,2)<perc_burn_in
        ...
        best_fit=[posterior_b(:,ind);VE(ind);AICBIC(ind).AIC;AICBIC(ind).BIC];%NB this line is obsolete?
        
%         % Best fit defined using avg of loglikelihood ???RR???
%         b=mean(posterior_latent_b(1,:),2);
%         %RR edit add AIC and BIC to best fit option
%         AICBIC=AICBIC(n_burn+1:end);
%         best_fit=[posterior_b(:,ind);VE(ind);AICBIC(ind).AIC;AICBIC(ind).BIC];
%         % end RR edit
